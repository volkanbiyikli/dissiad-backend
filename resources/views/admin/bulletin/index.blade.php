...
@section('pagelink')
    <a href="{{route('bulletin.create')}}" class="btn btn-danger kt-subheader__btn-options">Bülten Ekle</a>
@endsection
                        ...
                        <div class="kt-section">
                            <div class="kt-section__content">
                                <table class="table table-hover">
                                    ...
                                    <tbody id="sortable">
                                    @foreach($data['bulletin'] as $bulletin)
                                        <tr id="item-{{$bulletin->id}}">
                                            <td class="sortable align-middle">
                                                {{$bulletin['name']}}
                                            </td>
                                            <td class="align-middle kt-align-center kt-hidden-mobile">
                                                {{\Carbon\Carbon::parse($bulletin['date'])->isoFormat('DD MMMM YYYY')}}
                                            </td>
                                            <td class="align-middle kt-align-center kt-hidden-mobile">
                                                @if ($bulletin->status)
                                            ...
                                            <td class="align-middle kt-align-right">
                                                <a href="{{($settings->url)}}/bulten/{{($bulletin->slug)}}" target="_blank" class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                                   title="Görüntüle">
                                                    <i class="la la-chain"></i>
                                                </a>
                                                <a href="{{route('bulletin.edit',$bulletin->id)}}" class="btn btn-sm btn-clean btn-icon btn-icon-md"
                                                   title="Düzenle">
                                                    <i class="la la-edit"></i>
                                                </a>
                                                <a href="javascript:void(0)"
                                                   class="btn btn-sm btn-clean btn-icon btn-icon-md" title="Sil">
                                                    <i id="@php echo $bulletin->id @endphp" class="la la-trash"></i>
                                                </a>
                                            </td>
                                        </tr>
                                    @endforeach
      ...
      $(function() {
        ...
        $('.la-trash').click(function(e) {
          ...
          swal.fire({
            ...
            preConfirm         : () => {
              $.ajax({
                url    : 'bulletin/' + destroy_id,
                method : 'DELETE',
                success: function(msg) {
                  if(msg) {
                    $("#item-" + destroy_id).remove();
                    toastr.success("Bülten silme işlemi başarılı olarak gerçekleştirildi.", "Başarılı");
                  } else {
                    toastr.error("Bülten silme işlemi gerçekleştirilemedi.", "Başarısız");
                  }
                }
              })
            }
